 ...
 * from the cursor with a soft radial glow that follows it. Pure <canvas> + SVG,
 * no dependencies.
 *
 * Adapted for this app: no external CSS file (styles inlined), and pointer
 * tracking uses viewport coordinates so it works as a `position: fixed`
 * full-page background regardless of scroll.
 */
...
const DotField = memo(function DotField({
  ...
  const propsRef = useRef<Required<Omit<DotFieldProps, "glowRadius" | "glowColor" | "className" | "style">>>({} as never);
  propsRef.current = {
    dotRadius,
    dotSpacing,
    cursorRadius,
  ...
  useEffect(() => {
    ...
    function doResize() {
      ...
      // Viewport-relative offsets (fixed background → no scroll term).
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      // clientX/Y keeps the cursor aligned for a fixed, full-viewport canvas.
